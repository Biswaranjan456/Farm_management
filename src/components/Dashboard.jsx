  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl shadow-md border-l-4 border-green-500 hover:shadow-lg transition">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-md border-l-4 border-indigo-500 hover:shadow-lg transition">
          <div className="flex items-center gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-md border-l-4 border-purple-500 hover:shadow-lg transition">
          <div className="flex items-center gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-md border-l-4 border-blue-500 hover:shadow-lg transition">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-blue-100 text-blue-600 rounded-xl"><Sprout className="w-7 h-7" /></div>
            <div>
              <p className="text-gray-500 text-sm font-medium">Diary Entries</p>
              <p className="text-2xl font-bold text-gray-800">{diary.length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 w-full">
        {/* Expense Pie Chart */}
        <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-md border border-gray-100 overflow-hidden min-w-0 w-full">
          <h3 className="text-lg font-bold text-gray-800 mb-4">Expenses by Category</h3>
          {expenseByCategory.length > 0 ? (
            <div className="h-72 w-full min-w-0">
              <ResponsiveContainer width="99%" height="100%">
                <PieChart>
                  <Pie data={expenseByCategory} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
            ...
            <div className="h-72 flex items-center justify-center text-gray-400 bg-gray-50 rounded-xl">No expense data available</div>
          )}
        </div>

        {/* Inventory Bar Chart */}
        <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-md border border-gray-100 overflow-hidden min-w-0 w-full">
          <h3 className="text-lg font-bold text-gray-800 mb-4">Inventory Stock vs Min Required</h3>
          {inventoryData.length > 0 ? (
            ...
            <div className="h-72 flex items-center justify-center text-gray-400 bg-gray-50 rounded-xl">No inventory data available</div>
          )}
        </div>
      </div>
    </div>
